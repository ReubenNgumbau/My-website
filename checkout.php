			...
			<div class="col-75">
				...
				<form id="checkout_form" action="checkout_process.php" method="POST" class="was-validated">
					...
					while($i<=$total_count){
						...
						$total=$total+$amount_ ;
						$sql = "SELECT vehicle_id FROM vehicles WHERE vehicle_title='$item_name_'";
						$query = mysqli_query($con,$sql);
						$row=mysqli_fetch_array($query);
						$vehicle_id=$row["vehicle_id"];
						echo "	
						<input type='hidden' name='vehicle_id_$i' value='$vehicle_id'>
						<input type='hidden' name='vehicle_price_$i' value='$amount_'>
						<input type='hidden' name='vehicle_qty_$i' value='$quantity_'>
						";
						$i++;
					}
